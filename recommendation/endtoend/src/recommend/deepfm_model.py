...
# purchase_history_similarity -- 사용자의 기존 구매 상품과 후보 상품 간 임베딩 유사도.
#   구매 이력 없는 사용자(콜드스타트)는 0.0(중립)으로 처리되어 콜드스타트/기존 유저를
#   동일한 모델 구조로 함께 다룰 수 있다.
DENSE_FIELDS = [
    "age_months_norm", "weight_norm", "neutered", "bcs_norm", "age_group_ordinal",
    "price_norm", "age_fit_score", "purchase_history_similarity",
    "palatability_score", "digestion_score",
